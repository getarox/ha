...
const blockedHosts = /(^|\.)((facebook|twitter|x|instagram|linkedin|tiktok|snapchat|reddit)\.com|threads\.net)$/i;
function clean(items: Result[]) { const seen = new Set<string>(); return items.filter((x) => { try { const host = new URL(x.url).hostname; return x.url && !blockedHosts.test(host) && !seen.has(x.url) && seen.add(x.url); } catch { return false; } }).slice(0, 20); }
