          ...
          {user.role !== "admin" ? (
            ...
            <>
              ...
              <section className="grid gap-5 lg:grid-cols-[1.1fr_.9fr]">
                <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-6"><div className="flex items-center gap-3"><ShieldCheck className="h-5 w-5 text-cyan-300" /><h2 className="text-lg font-medium">هوية المنصة</h2></div><dl className="mt-6 space-y-4 text-sm"><div className="flex justify-between gap-4 border-b border-white/10 pb-3"><dt className="text-slate-500">العقل</dt><dd className="text-left text-slate-200">{statsQuery.data.identity}</dd></div><div className="flex justify-between gap-4 border-b border-white/10 pb-3"><dt className="text-slate-500">المطور</dt><dd className="text-slate-200">{statsQuery.data.developer}</dd></div><div className="flex justify-between gap-4"><dt className="text-slate-500">الموقع الرسمي</dt><dd><a className="inline-flex items-center gap-1 text-cyan-300 hover:text-white" href={statsQuery.data.officialSiteUrl} target="_blank" rel="noreferrer">فتح الموقع <ArrowUpRight className="h-3.5 w-3.5" /></a></dd></div></dl></div>
