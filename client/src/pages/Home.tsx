...
export default function Home() {
  ...
  const hasRealOAuthUser = user?.loginMethod === "google" || user?.loginMethod === "microsoft" || user?.loginMethod === "apple";
  ...
  const acceptAgreement = async () => {
    if (!hasRealOAuthUser) { startLogin(); return; }
  ...
  return (
    <div className="aurevion-shell min-h-screen overflow-hidden text-white" dir={consent.locale === "en" ? "ltr" : "rtl"}>
      ...
      {!consent.loading && (!consent.accepted || !hasRealOAuthUser) && consent.agreement && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/95 p-4" role="dialog" aria-modal="true"><div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-cyan-300/25 bg-slate-950 p-6 text-slate-100 shadow-2xl sm:p-8"><div className="flex items-start justify-between gap-4"><div><p className="text-xs uppercase tracking-[0.2em] text-cyan-300/70">AUREVION / REQUIRED CONSENT</p><h1 className="mt-2 text-2xl font-semibold text-white">{consent.agreement.title}</h1></div><select aria-label="لغة الاتفاقية" value={consent.locale} onChange={(event) => setConsent((current) => ({ ...current, locale: event.target.value as "ar" | "en" }))} className="rounded-lg border border-white/10 bg-white/10 p-2 text-sm"><option value="ar">العربية</option><option value="en">English</option></select></div>{!hasRealOAuthUser && <div className="mt-5 rounded-lg border border-cyan-300/20 bg-cyan-300/5 p-4 text-sm leading-7 text-cyan-100">يجب أولًا تسجيل الدخول بحساب حقيقي عبر Google أو Microsoft أو Apple. بعد تسجيل الدخول ستعود إلى هذه الاتفاقية، ولن يعمل أي أمر قبل تحقق الشرطين.</div>}<p className="mt-5 text-sm leading-7 text-slate-300">{consent.agreement.intro}</p><ul className="mt-5 list-disc space-y-3 ps-5 text-sm leading-7 text-slate-300">{consent.agreement.bullets.map((bullet) => <li key={bullet}>{bullet}</li>)}</ul><p className="mt-6 rounded-lg border border-amber-300/20 bg-amber-300/5 p-3 text-xs text-amber-100">{consent.agreement.required}</p><div className="mt-6 flex flex-col gap-3 sm:flex-row"><Button type="button" onClick={() => void acceptAgreement().catch((error) => window.alert(error.message))} className="bg-cyan-300 text-slate-950 hover:bg-cyan-200">{hasRealOAuthUser ? consent.agreement.accept : "تسجيل الدخول بحساب Google / Microsoft / Apple"}</Button><Button type="button" variant="outline" onClick={() => window.location.href = "about:blank"} className="border-white/15">{consent.agreement.reject}</Button></div><p className="mt-4 text-[11px] text-slate-500">Consent version: 2026-09-13.v1</p></div></div>}
      <header className="aurevion-header relative z-10 mx-auto flex w-full max-w-7xl items-center justify-between px-5 py-5 lg:px-8">
        ...
        <div className="flex items-center gap-2">
          ...
          <aside id="aurevion-settings-sidebar" className={`fixed end-0 top-0 z-[60] flex h-full w-[min(88vw,20rem)] max-w-[22rem] flex-col border-s border-cyan-300/20 bg-slate-950/98 p-5 text-sm text-slate-200 shadow-2xl transition-[transform,opacity,visibility] duration-300 ease-out ${menuOpen ? "visible pointer-events-auto translate-x-0 opacity-100" : "invisible pointer-events-none translate-x-full opacity-0"}`} aria-hidden={!menuOpen}>
