...
function ImageAttachment({ src }: { src: string }) {
  ...
  return <div className="relative mt-3 min-h-48 w-full overflow-hidden rounded-xl border border-white/10 bg-slate-900/60">
    {!loaded && !failed && <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-slate-900 via-slate-800 to-cyan-950/40"><div className="w-3/4 space-y-3"><div className="h-3 animate-pulse rounded bg-white/10" /><div className="h-3 w-2/3 animate-pulse rounded bg-white/10" /><p className="text-center text-xs text-slate-500">جارٍ تجهيز الصورة…</p></div></div>}
    {failed ? <div className="flex min-h-48 items-center justify-center p-4 text-xs text-slate-400">تعذر تحميل الصورة. أعد المحاولة لإظهارها.</div> : <img src={src} alt="صورة منشأة داخل المحادثة" loading="eager" onLoad={() => setLoaded(true)} onError={() => setFailed(true)} className={cn("relative z-10 max-h-[28rem] w-full object-contain transition-opacity duration-300", loaded ? "opacity-100" : "opacity-0")} />}
  </div>;
}
